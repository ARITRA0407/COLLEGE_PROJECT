...
class HybridCareerRecommender:
    """
    Hybrid quiz-career recommender.

    It preserves the original rule-based coverage idea and adds:
    1. Direct vector similarity over the user's skill profile.
    2. PCA latent-factor similarity to capture correlated skills.
    3. A tree-based reranker trained on synthetic user profiles generated
       around the career archetypes defined in CAREER.csv.
    """
        ...
        for role_index, vector in enumerate(vectors):
            ...
            for _ in range(samples_per_role):
                ...
                if len(neighbor_indices) and rng.random() < 0.35:
                    ...
                    sample = (blend * vector) + ((1.0 - blend) * neighbor) + (noise * 0.5)
    ...
    def _fit_models(self):
        ...
        try:
            ...
            self.model_metrics = {
                ...
                "macro_f1": round(float(f1_score(y_test, ensemble_pred, average="macro")), 4),
                "log_loss": round(float(log_loss(y_test, ensemble_probs, labels=np.arange(len(self.job_names)))), 4),
            }
        except Exception:
            self.scaler = None
            self.pca = None
    ...
    def _latent_similarity(self, user_vector):
        if not SKLEARN_AVAILABLE or self.scaler is None or self.pca is None or self.prototype_latent is None:
    ...
    def _rule_score(self, user_levels, role_vector):
        user_level_vector = np.array([
            LEVEL_TO_SCORE.get(str(user_levels.get(col, "Low")).strip().lower(), 0.25)
    ...
    def _alignment_score(self, user_vector, role_vector):
        return float(np.clip(1.0 - np.mean(np.abs(user_vector - role_vector)), 0.0, 1.0))
    ...
    def recommend(self, user_scores, user_levels, category_metrics, top_n=5):
        ...
        cosine_scores = np.dot(self._normalize_rows(self.prototype_matrix), self._normalize_rows([user_vector])[0])
