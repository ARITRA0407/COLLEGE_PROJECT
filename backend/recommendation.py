...
class CollegeRecommender:
    ...
    - historical rank heuristics
    - Apriori-style association rules
    - cached tree/ensemble rank regressors
    ...
    def _prepare_master_rank_df(self):
        ...
        for df_name, df in self.dataframes.items():
            ...
            if "Stream" in tmp.columns and not tmp["Stream"].empty:
                tmp["Stream"] = tmp["Stream"].fillna("").astype(str).str.replace(
    ...
    def _prepare_quality_data(self):
        ...
        self.full_college_df = self.full_college_df.rename(columns={"logo_image_url": "logo_image"})
        if not self.full_college_df.empty:
            expected_cols = ["Institute", "District", "Location", "Website", "logo_image", "Picture"]
    ...
    def _predict_top_colleges_rank_only(
        ...
        def apply_filters(df_in, prog, strm, qta, cat, dist):
            ...
            if prog:
                ...
                tmp = exact if not exact.empty else tmp[tmp["Program"].str.contains(prog, na=False)].copy()
        ...
        for name, group in final_ranks.groupby(grouping_cols):
            ...
            recent = [float(x) for x in sorted_group["Closing Rank"].head(2).tolist() if pd.notna(x)]
            if not recent:
                continue
            ...
            prediction_results.append(
                {
                    ...
                    "Opening Rank": latest_data["Opening Rank"] if pd.notna(latest_data["Opening Rank"]) else np.nan,
                    "Seat Type": latest_data["Seat Type"] if "Seat Type" in latest_data.index else "",
                    "District": latest_data["District"] if "District" in latest_data.index else "",
                }
            )
    ...
    def _merge_quality_columns(self, df):
        ...
        if not getattr(self, "combined_quality_df", pd.DataFrame()).empty:
            merged = pd.merge(df, self.combined_quality_df, on=["Institute", "Program"], how="left")
        else:
            merged = df.copy()
    ...
    def _filter_top_colleges_by_metrics(self, df, min_ctc, min_placements_score=0):
        ...
        if min_ctc > 0 and "Max Average CTC" in filtered_df.columns:
            filtered_df = filtered_df[pd.to_numeric(filtered_df["Max Average CTC"], errors="coerce").fillna(0) >= min_ctc]
        if min_placements_score > 0 and "placements_score_filter" in filtered_df.columns:
            filtered_df = filtered_df[
                pd.to_numeric(filtered_df["placements_score_filter"], errors="coerce").fillna(0) >= min_placements_score
            ]
        return filtered_df
    ...
    def _compute_boosts_from_rules(self, candidates_df, user_filters):
        ...
        for _, rule in rules_df.iterrows():
            ...
            try:
                boost_value = float(rule.get("confidence", 0.0)) * float(rule.get("support", 0.0))
            except Exception:
                boost_value = 0.0
    ...
    def _build_group_predictions_all(self):
        ...
        for name, group in df.groupby(grouping_cols):
            ...
            recent = [float(x) for x in valid_group.sort_values("Year", ascending=False)["Closing Rank"].head(2).tolist() if pd.notna(x)]
            if not recent:
                continue
            ...
            prediction_rows.append(
                {
                    ...
                    "Category": name[4],
                    "Predicted Closing Rank": max(1.0, float(np.mean(recent))),
                    "Latest Closing Rank": float(latest_row["Closing Rank"]) if pd.notna(latest_row["Closing Rank"]) else np.nan,
                    "Opening Rank": float(latest_row["Opening Rank"]) if pd.notna(latest_row["Opening Rank"]) else np.nan,
                }
            )
    ...
    def _build_training_frame(self):
        ...
        if not getattr(self, "combined_quality_df", pd.DataFrame()).empty:
            grouped_df = pd.merge(grouped_df, self.combined_quality_df, on=["Institute", "Program"], how="left")

        for col in self.MODEL_FEATURE_NUMERIC:
            if col not in grouped_df.columns:
                grouped_df[col] = 0.0
            grouped_df[col] = pd.to_numeric(grouped_df[col], errors="coerce").fillna(0.0)
    ...
    def _fit_or_transform_encoder(self, series, column_name, fit=False):
        clean_series = series.astype(str).fillna("").str.strip().str.lower().replace("nan", "")
        ...
        if fit:
            ...
            if encoder is None:
                return pd.Series([0] * len(clean_series), index=clean_series.index, dtype=int)

        known_values = set(self.label_encoders[column_name].classes_)
        clean_series = clean_series.where(clean_series.isin(known_values), unknown_token)
        return pd.Series(
    ...
    def _regression_metrics(self, y_true, y_pred):
        ...
        rmse = float(np.sqrt(mean_squared_error(y_true, y_pred))) if mean_squared_error else None
        ...
        return {
            ...
            "mae": round(float(mean_absolute_error(y_true, y_pred)), 4) if mean_absolute_error else None,
            "rmse": round(rmse, 4) if rmse is not None else None,
            "r2": round(float(r2_score(y_true, y_pred)), 4) if r2_score else None,
            "median_ae": round(float(median_absolute_error(y_true, y_pred)), 4) if median_absolute_error else None,
    ...
    def _prepare_ml_assets(self):
        ...
        for model_name, model in candidate_models.items():
            try:
                ...
                preds = np.clip(np.asarray(model.predict(X_test), dtype=float), 1.0, None)
        ...
        self.hybrid_model_weights = {
            name: round(value / total_weight, 6)
    ...
    def _apply_hybrid_ranking(self, df, user_rank_val, rule_boosts, effective_rank_col):
        ...
        ranked_df["Rule Boost"] = ranked_df.apply(lambda row: rule_boosts.get(key_for_row(row), 0.0), axis=1)
        ranked_df["Rule Boost Score"] = self._normalize_series(ranked_df["Rule Boost"])

        quality_ctc = self._normalize_series(ranked_df.get("Max Average CTC", pd.Series(index=ranked_df.index, dtype=float)))
        quality_place = self._normalize_series(
            ranked_df.get("placements_score_filter", pd.Series(index=ranked_df.index, dtype=float))
        )
        ...
        if max_gap <= 0:
            accessibility = pd.Series([1.0] * len(ranked_df), index=ranked_df.index, dtype=float)
        else:
    ...
    def _finalize_table(self, ranked_filtered_df, effective_rank_col):
        rank_col = effective_rank_col if effective_rank_col in ranked_filtered_df.columns else "Predicted Closing Rank"
        base_cols = [
            "Institute",
            "Program",
            "Stream",
        ...
        final_df["Closing Rank"] = pd.to_numeric(ranked_filtered_df[rank_col], errors="coerce").fillna(0.0)

        if not getattr(self, "full_college_df", pd.DataFrame()).empty:
            final_df = pd.merge(final_df, self.full_college_df, on="Institute", how="left", suffixes=("", "_college"))
            if "District_college" in final_df.columns:
                final_df["District"] = final_df["District"].replace("", np.nan).fillna(final_df["District_college"])
        ...
        if not getattr(self, "full_placement_df", pd.DataFrame()).empty:
            final_df = pd.merge(final_df, self.full_placement_df, on=["Institute", "Program"], how="left")
    ...
    def recommend(
        ...
        working_df, effective_rank_col = self._attach_hybrid_rank_predictions(working_df)

        ranked_results_df = working_df[
            pd.to_numeric(working_df[effective_rank_col], errors="coerce").fillna(0.0) >= user_rank_val
        ].copy()
        ranked_results_df = ranked_results_df.sort_values(by=effective_rank_col, ascending=True)
        ...
        final_table_candidates = self._finalize_table(ranked_results_df, effective_rank_col=effective_rank_col)
