"""
Grid-search the hand-picked weighted-sum equations used by the backend.

Run from the project root:
    python backend/weight_calibration.py

Outputs:
    results/calibrated_weight_search.txt   human-readable tables
    results/calibrated_weights.json        runtime weights used by app/recommenders
"""
...
from calibrated_weights import DEFAULT_WEIGHT_CONFIG  # noqa: E402
...
def _classification_metrics(y_true, scores, probabilities=False):
    ...
    top3_accuracy = float(np.mean([y_true[idx] in top_indices[idx] for idx in range(len(y_true))]))
...
def _evaluate_probability_grid(probabilities, y_true, names, step, top_n=25):
    ...
    for weights in _simplex_grid(names, step):
        ...
        rows.append({
            "weights": {key: round(float(value), 6) for key, value in weights.items()},
            "metrics": metrics,
            "sort_key": (-metrics["accuracy"], metrics["log_loss"], -metrics["macro_f1"]),
        })
    rows.sort(key=lambda item: item["sort_key"])
    return rows[:top_n], len(rows)
...
def _evaluate_multiclass_score_grid(components, y_true, names, step, top_n=25):
    rows = []
    for weights in _simplex_grid(names, step):
        scores = np.tensordot(components, _weights_to_array(weights, names), axes=([2], [0]))
...
def _calibrate_college_model_ensemble(recommender, training_df):
    ...
    if not SKLEARN_AVAILABLE or train_test_split is None:
        return [], 0, "scikit-learn is not available, so DT/RF/GB validation predictions cannot be generated."
    if training_df.empty or len(training_df) < 20:
        return [], 0, "not enough college rank training rows."

    available_names = [name for name in names if name in getattr(recommender, "trained_models", {})]
    ...
    try:
        ...
        y_test = pd.to_numeric(test_df["Latest Closing Rank"], errors="coerce").fillna(0.0).to_numpy(dtype=float)

        predictions = []
        for model_name in available_names:
            model = recommender.trained_models[model_name]
            predictions.append(np.clip(np.asarray(model.predict(X_test), dtype=float), 1.0, None))
...
def _quality_components(training_df):
    ...
    target = _normalise(training_df.get("Latest Closing Rank", 0.0), invert=True, log_scale=True)
...
def _calibrate_college_recommendation(recommender, training_df, quality_weights):
    ...
    latest = pd.to_numeric(training_df.get("Latest Closing Rank", 0.0), errors="coerce").fillna(0.0).to_numpy(dtype=float)
    predicted = pd.to_numeric(training_df.get("Predicted Closing Rank", 0.0), errors="coerce").fillna(0.0).to_numpy(dtype=float)
...
def _load_quiz_model():
    try:
        ...
        career_df = pd.read_csv(PROJECT_ROOT / "csv" / "CAREER.csv", encoding="utf-8-sig")
...
def _quiz_validation_data(quiz_model, samples_per_role=160):
    ...
    try:
        X, y = quiz_model._build_synthetic_training_data(samples_per_role=samples_per_role)
...
def _calibrate_quiz_model_ensemble(quiz_model, X_test, y_test):
    ...
    if getattr(quiz_model, "rf_model", None) is None or getattr(quiz_model, "gb_model", None) is None:
...
def _career_components(quiz_model, X_test, quiz_model_weights):
    ...
    if getattr(quiz_model, "scaler", None) is not None and getattr(quiz_model, "pca", None) is not None:
        latent_user = _normalise_rows(quiz_model.pca.transform(quiz_model.scaler.transform(user_matrix)))
    ...
    if getattr(quiz_model, "rf_model", None) is not None and getattr(quiz_model, "gb_model", None) is not None:
    ...
    user_levels = np.where(user_matrix >= 0.75, 0.90, np.where(user_matrix >= 0.45, 0.60, 0.25))
    rule = np.mean(user_levels[:, None, :] >= role_matrix[None, :, :], axis=2)
    readiness = 1.0 - np.mean(np.maximum(role_matrix[None, :, :] - user_matrix[:, None, :], 0.0), axis=2)
...
def _simulate_skill_rows(q_df, samples_per_category=90):
    ...
    for category, group in q_df.groupby("Category"):
        ...
        for _ in range(samples_per_category):
            ...
            selected_indices = rng.choice(len(group), size=attempts, replace=len(group) < attempts)
            ...
            for question_index in selected_indices:
                ...
                if max_weight <= 0:
                    max_weight = 1.0
                normalized_options = np.asarray(option_weights, dtype=float) / max_weight
                ...
                time_factor = float(np.clip(1.05 - (difficulty * 0.25) - (abs(normalized_score - ability) * 0.55) + rng.normal(0, 0.08), 0.30, 1.0))
                correct = normalized_score >= max(normalized_options)
                confidence = float(np.clip(0.25 + (0.65 * ability) + rng.normal(0, 0.12), 0.0, 1.0))
            ...
            confidence_alignment = float(np.mean(confidence_alignments)) if confidence_alignments else 0.5
            bayesian_skill = float(skill_values[-1]) if skill_values else 0.5
            consistency = float(np.clip(1.0 - min(np.std(observed) / 0.35, 1.0), 0.0, 1.0)) if len(observed) > 1 else 0.5
            deltas = np.diff(skill_values) if len(skill_values) > 1 else np.asarray([0.0])
            momentum = float(np.clip(0.5 + (float(np.mean(deltas)) * 3.0), 0.0, 1.0))
            challenge_index = float(np.mean(challenge_values)) if challenge_values else 0.0
            uncertainty = (
                (1.0 - bayesian_skill) * 0.45
                + (1.0 - confidence_alignment) * 0.35
                + (1.0 - time_efficiency) * 0.20
            )
            target_uncertainty = float(np.clip(
            ...
            target_reliability = float(np.clip(1.0 - abs(bayesian_skill - ability), 0.0, 1.0))
            rows.append({
                "category": category,
                "target": ability,
...
def main():
    ...
    training_df = _college_training_frame(recommender) if recommender is not None else pd.DataFrame()

    rows, tested, skipped = ([], 0, college_error) if recommender is None else _calibrate_college_model_ensemble(recommender, training_df)
    sections.extend(_format_section(
    ...
    best = _best_payload(rows, "lowest validation RMSE, then MAE", "rank model validation split", tested)
    if best:
    ...
    best = _best_payload(rows, "lowest RMSE against historical rank desirability", "college rank and quality CSVs", tested)
    if best:
    ...
    rows, tested, skipped = ([], 0, college_error) if recommender is None else _calibrate_college_recommendation(
        recommender,
        training_df,
    ...
    best = _best_payload(rows, "lowest RMSE against historical rank desirability", "college rank, quality, and association-rule proxies", tested)
    if best:
        payload["weights"]["college_recommendation_score"] = best

    quiz_model, _, quiz_error = _load_quiz_model()
    X_test, y_test, quiz_validation_error = (None, None, quiz_error) if quiz_model is None else _quiz_validation_data(quiz_model)

    rows, tested, skipped = ([], 0, quiz_validation_error) if quiz_validation_error else _calibrate_quiz_model_ensemble(quiz_model, X_test, y_test)
    sections.extend(_format_section(
    ...
    best = _best_payload(rows, "highest accuracy, then lowest log loss", "synthetic career-profile validation split", tested)
    if best:
    ...
    rows, tested, skipped = ([], 0, quiz_validation_error) if quiz_validation_error else _calibrate_career_score(
        quiz_model,
        X_test,
        y_test,
    ...
    best = _best_payload(rows, "highest top-1 accuracy, then lowest log loss", "synthetic career-profile validation split", tested)
    if best:
    ...
    best = _best_payload(rows, "lowest RMSE against simulated skill ability", "QUESTIONS.csv synthetic quiz sessions", tested)
    if best:
    ...
    best = _best_payload(rows, "lowest RMSE against simulated uncertainty", "QUESTIONS.csv synthetic quiz sessions", tested)
    if best:
    ...
    best = _best_payload(rows, "lowest RMSE against simulated reliability", "QUESTIONS.csv synthetic quiz sessions", tested)
    if best:
    ...
    best = _best_payload(rows, "lowest RMSE against simulated skill ability", "QUESTIONS.csv synthetic quiz answer steps", tested)
    if best:
    ...
    sections.append("------------------------")
    sections.append("Cosine similarity, TF-IDF, accuracy, precision, recall, F1, and normalization are mathematical definitions, not arbitrary project weight vectors.")
    sections.append("Review keyword weights are lexicon scores rather than normalized weighted-sum coefficients, so they are not changed by this calibration file.")
