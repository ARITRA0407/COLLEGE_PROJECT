(function () {
  ...
  function initMap(lat, lon) {
    try {
      ...
      el.innerHTML = '<div style="padding:12px;font-size:13px;color:#6b7280">Map unavailable</div>';
    }
  }
  ...
  async function loadDetails(name) {
    ...
    try {
      ...
      if (d.latitude && d.longitude) {
        ids.coordLine.textContent = `Lat: ${d.latitude.toFixed(6)} • Lon: ${d.longitude.toFixed(6)}`;
      ...
      ids.placementAvg.textContent = placement.avg_ctc || d.placement_summary?.avg_ctc || "—";
      ids.placementMed.textContent = placement.median_ctc || d.placement_summary?.median_ctc || "—";
      ids.placementHigh.textContent = placement.highest_ctc || d.placement_summary?.highest_ctc || "—";
      ids.placementRating.textContent = placement.placement_rating || d.placement_summary?.placement_rating || "—";

      const rlist = (reviews.reviews || []).slice(0, 50);
      if (rlist.length === 0) {
        ids.reviewsList.innerHTML = '<div class="muted">No reviews in reviews.csv for this institute.</div>';
      } else {
